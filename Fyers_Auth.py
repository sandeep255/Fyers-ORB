...
username = "XXXXXX"           # fyers_id
password = "******"           # fyers_password
pin =   0000                  # your integer pin
client_id =  ""               # From the Fyers API app
secret_key = ''               # From the Fyers API app 
...
def check():
    try:
        ...
        sys.exit()
    fyers = fyersModel.FyersModel(client_id=client_id, token=token, log_path=os.getcwd())
    response = fyers.get_profile() # This is a test to see if the auth code is working. You can comment it out once its confirmed.
    if 'error' in response['s'] or 'error' in response['message'] or 'expired' in response['message']:
        ...
        # print(response)   # In case you want to  check the response
